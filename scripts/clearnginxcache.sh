#!/bin/bash
#
# Simple clear cache for nginx-more, clear FCGI, Proxy and PageSpeed cache -- Version 1.0
...
if [ ! -d "$CDIR1" ] || [ ! -d "$CDIR2" ] || [ ! -d "$CDIR3" ] || [ ! -d "$CDIR4" ] || [ ! -d "$CDIR5" ];then
	echo -e "\nError: Directory $CDIR1 or $CDIR2 or $CDIR4 or $CDIR5 doesn't seem to exist. Update directories in the script.\n"
	exit 0
fi
