...
EMAILS="" ## Separate each email with space
...
WWWPASS=$(tr -cd 'a-f0-9' < /dev/urandom | head -c 8)
SQLPASS=$(tr -cd 'a-f0-9' < /dev/urandom | head -c 8)
MONITPASS=$(tr -cd 'a-f0-9' < /dev/urandom | head -c 8)
PMA=$(tr -cd 'a-f0-9' < /dev/urandom | head -c 8)
MariaDB=false
...
if [ $PHPVERSION == "53" ]; then
	{
		...
		yum --enablerepo ius-archive install -y php53u php53u-cli php53u-common php53u-devel php53u-enchant php53u-gd php53u-imap php53u-ioncube-loader php53u-mbstring php53u-mcrypt php53u-mysql php53u-pdo php53u-pear php53u-pecl-memcache php53u-pecl-memcached php53u-soap php53u-tidy php53u-xml php53u-xmlrpc
...
elif [ $PHPVERSION == "55" ]; then
	{
		yum install -y $URL/repos/ius-release-6.noarch.rpm
		yum install -y yum-plugin-replace
		yum install -y php55u php55u-cli php55u-common php55u-devel php55u-enchant php55u-gd php55u-imap php55u-ioncube-loader php55u-opcache php55u-mbstring php55u-mcrypt php55u-mysql php55u-pdo php55u-pear php55u-pecl-memcache php55u-pecl-memcached php55u-soap php55u-tidy php55u-xml php55u-xmlrpc
	} 3>&1 4>&2 >>$BUILDLOG 2>&1
elif [ $PHPVERSION == "56" ]; then
	{
		yum install -y $URL/repos/ius-release-6.noarch.rpm
		yum install -y yum-plugin-replace
		yum install -y php56u php56u-cli php56u-common php56u-devel php56u-enchant php56u-gd php56u-imap php56u-ioncube-loader php56u-opcache php56u-mbstring php56u-mcrypt php56u-mysql php56u-pdo php56u-pear php56u-pecl-memcache php56u-pecl-memcached php56u-soap php56u-tidy php56u-xml php56u-xmlrpc
	} 3>&1 4>&2 >>$BUILDLOG 2>&1
fi
...
gpgcheck=1" > /etc/yum.repos.d/MariaDB.repo
	...
	{
		yum install -y mysql
		yum replace -y mysql --replace-with mysql56u
		yum install -y mysql56u-server
	} 3>&1 4>&2 >>$BUILDLOG 2>&1
fi
...
rm -rf /home/www/"$DOMAIN"/subdomains/pma/setup 3>&1 4>&2 >>$BUILDLOG 2>&1
...
for service in $SERVICES; do
	/sbin/chkconfig --level 123456 "$service" off 3>&1 4>&2 >>$BUILDLOG 2>&1
done
...
if [ "$VIRT" == "openvz" ] || [ "$VIRT" == "xen" ] || [ "$VIRT" == "kvm" ] || [ "$VIRT" == "vmware" ]; then
	...
	chmod 700 /opt/scripts/mysqltuner.pl 3>&1 4>&2 >>$BUILDLOG 2>&1
